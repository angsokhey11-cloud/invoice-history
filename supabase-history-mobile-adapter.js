/* BIG BROTHER — Mobile Invoice History Role/Location Scope Supabase Adapter V1 */
(function(){
  'use strict';
  ...
  let session=null;
  const READ_ONLY=new URLSearchParams(location.search).get('mobileReadOnly')==='1';
  ...
  async function fetchApi(params={}){
    ...
    if(action==='verifyInvoiceHistoryPrimaryAdmin'){
      if(READ_ONLY)return {success:true,authorized:false,readOnly:true};
      const p=await rpc('bb_current_access_profile');
      return {success:true,authorized:p?.user?.isAdmin===true};
    }
    if(action==='invoiceHistoryRevision'){const d=await rpc('bb_invoice_history_mobile_revision');return {success:true,revision:String(d||'0')}}
    if(action==='invoiceDetail'){
      const invoiceId=String(params.invoiceId||'').trim();
      if(invoiceId)return rpc('bb_invoice_history_mobile_detail_by_id',{p_invoice_id:invoiceId});
      return rpc('bb_invoice_history_mobile_detail',{p_invoice_no:String(params.invoiceNo||'')});
    }
    if(action==='invoiceList')return rpc('bb_invoice_history_mobile_list',{
  ...
  async function historyJsonp(_url,params={}){
    const d=await rpc('bb_invoice_history_mobile_master');
    const action=String(params.action||'');
  ...
  async function detailById(invoiceId){
    return rpc('bb_invoice_history_mobile_detail_by_id',{p_invoice_id:String(invoiceId||'').trim()});
  }
  ...
  async function postHistoryAction(action,_fieldName,payload){
    if(READ_ONLY)throw new Error('This Mobile Invoice view is read-only.');
  ...
  async function requirePrimaryAdmin(){
    if(READ_ONLY){alert('This Mobile Invoice view is read-only.');return false;}
    const p=await rpc('bb_current_access_profile');
